...
module KnoxTrain
  class CLI < Thor
    ...
    def status
      ...
      reg.profiles.each_value do |profile|
        backends = profile.backends
        backends = backends.select { |b| b.type == backend_filter } if backend_filter
        backends.each do |backend|
    ...
    def print_status_table(rows)
      ...
      header = %w[Profile Backend Snaps Latest Files Restore Stored Ratio]
      data   = rows.map { |r| status_row(r) }
      table  = TTY::Table.new(header: header, rows: data)
      puts table.render(:unicode,
                        alignment: %i[left left right left right right right right])
    end
    ...
    def status_row(row)
      ratio = row.stored_bytes.positive? ? format('%.2fx', row.restore_bytes.to_f / row.stored_bytes) : '—'
      [
        row.profile_name, row.backend_type,
        row.snapshot_count,
        row.latest_time ? fmt_time(row.latest_time) : 'never',
        fmt_number(row.file_count),
        fmt_bytes(row.restore_bytes),
        fmt_bytes(row.stored_bytes),
        ratio
      ]
    end
    ...
    def fmt_bytes(bytes)
      ...
      %w[B KiB MiB GiB TiB].each do |unit|
        return format('%.1f %s', f, unit) if f < 1024
