...
class SchedulerLaunchdTest < Minitest::Test
  ...
  def stub_launchctl(agent)
    calls = []
    agent.define_singleton_method(:launchctl) { |sub, path| calls << [sub, path]; true }
    calls
  end
  ...
  def test_install_writes_plist_with_label
    Dir.mktmpdir do |dir|
      ...
      stub_launchctl(agent); stub_log_dir(agent)
      agent.install
  ...
  def test_install_embeds_hour_and_minute
    Dir.mktmpdir do |dir|
      ...
      assert_match(/<integer>3<\/integer>/, content)
      assert_match(/<integer>30<\/integer>/, content)
    end
  end
  ...
  def test_install_includes_backup_all_arguments
    Dir.mktmpdir do |dir|
      ...
      agent.install
      content = File.read(agent.plist_path)
      assert_match(/<string>backup<\/string>/, content)
      assert_match(/<string>--all<\/string>/, content)
    end
  end
  ...
  def test_install_calls_launchctl_load
    Dir.mktmpdir do |dir|
      agent = make_agent
      agent.define_singleton_method(:plist_path) { File.join(dir, "test.plist") }
      calls = stub_launchctl(agent); stub_log_dir(agent)
      agent.install
